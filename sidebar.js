...
function exportPDF() {
  ...
  img.onload = function () {
    ...
    // --- Table Header ---
    let y = 55;
    const colDate = pageWidth * 0.15; // New Date column (far left)
    const col1 = pageWidth * 0.35; // Park Number
    const col2 = pageWidth * 0.6;  // Date Completed (existing)
    const col3 = pageWidth * 0.85; // Notes

    doc.setFontSize(12);
    doc.text("Date", colDate, y, { align: "center" });
    ...
    for (const parkId in userData) {
      const entry = userData[parkId];
      const date = entry.date || "";
      const completed = entry.completed ? entry.date : "";
      const notes = entry.notes || "";

      doc.text(date, colDate, y, { align: "center" });
      doc.text(parkId, col1, y, { align: "center" });
      doc.text(completed, col2, y, { align: "center" });
      doc.text(notes, col3, y, { align: "center" });
