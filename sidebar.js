...
// Export sidebar data to a real PDF (reads from localStorage for reliability)
function exportPDF() {
  try {
    if (!window.jspdf || !window.jspdf.jsPDF) {
      alert("PDF library not loaded. Please ensure jsPDF is included on the page.");
      return;
    }
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF();

    // Load latest data directly from localStorage (not the closed-over variable)
    let store = {};
    try { store = JSON.parse(localStorage.getItem("parkData") || "{}"); } catch (e) { store = {}; }
    ...
    if (keys.length === 0) {
      ...
      keys.forEach((id, idx) => {
        const done = store[id] && store[id].done ? "✔ Done" : "✘ Not Done";
        const note = (store[id] && store[id].note) ? String(store[id].note) : "";
        const time = (store[id] && store[id].time) ? new Date(store[id].time).toLocaleString() : "";
        const line = `${id} - ${done}${time ? " (" + time + ")" : ""}${note ? " - Notes: " + note : ""}`;
...
// Clear all user data
// Clear all user data (localStorage) and reload
...
// Show parks completed today (reads from localStorage)
function showToday() {
  try {
    let store = {};
    try { store = JSON.parse(localStorage.getItem("parkData") || "{}"); } catch (e) { store = {}; }
    ...
    Object.keys(store).forEach(id => {
      const item = store[id] || {};
      if (item.done && item.time && String(item.time).startsWith(today)) {
...
" + completedToday.join(", "));
    ...
    alert("Failed to compute today's completions. See console for details.");
  }
}
