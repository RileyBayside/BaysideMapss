...
function exportPDF() {
  ...
  // --- Group by Zone ---
  Object.keys(window.zones || {}).forEach(zone => {
    const zoneParks = window.zones[zone];

    const relevant = zoneParks.filter(id => parks[id]); // only those with data
