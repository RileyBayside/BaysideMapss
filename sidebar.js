...
M0753`
  ...
  // Storage
  let data = JSON.parse(localStorage.getItem(PARK_DATA_KEY) || "{}");
  let nameIndex = {}; // MowingID -> Name

  function saveData() {
    localStorage.setItem(PARK_DATA_KEY, JSON.stringify(data));
  }
  ...
  function renderSidebar() {
    ...
    Object.keys(zones).forEach(zone => {
      ...
      zones[zone].forEach((parkId, idx) => {
        ...
        checkbox.checked = data[parkId]?.done || false;
        checkbox.onchange = () => {
          data[parkId] = data[parkId] || {};
          data[parkId].done = checkbox.checked;
          data[parkId].time = new Date().toISOString();
          saveData();
        };
        ...
        note.type = "text";
        note.value = data[parkId]?.note || "";
        note.placeholder = "Notes...";
        note.onchange = () => {
          data[parkId] = data[parkId] || {};
          data[parkId].note = note.value;
          saveData();
        };
...
function exportPDF(data) {
  ...
  if (keys.length === 0) {
    ...
    keys.forEach(id => {
      const done = data[id].done ? "✔ Done" : "✘ Not Done";
      const note = data[id].note || "";
      const line = `${id} - ${done}${note ? " - Notes: " + note : ""}`;
