...
function exportPDF() {
  ...
  // --- Add Logo ---
  const img = new Image();
  img.src = "logo.png"; // Ensure logo.png is in same folder
  img.onload = function() {
    const imgWidth = 40; // adjust size as needed
    const imgHeight = 20;
    ...
    keys.forEach(id => {
      ...
      doc.text(id, 20, y);
      doc.text(date, 80, y);
      doc.text(note, 140, y, { maxWidth: 60 }); // wrap notes column

      y += 7;
      if (y > 280) { // if near bottom, create new page
        doc.addPage();
        y = 20;
      }
    });
    ...
    keys.forEach(id => {
      const done = __getParkData()[id].done ? "✔ Done" : "✘ Not Done";
      const note = __getParkData()[id].note || "";
