...
function __getParkData() {
  ...
  // Now, also pull zone info from GeoJSON features
  if (window.geojsonData && window.geojsonData.features) {
    window.geojsonData.features.forEach(feature => {
      const desc = feature.properties?.description?.value || "";
      const idMatch = desc.match(/MowingID<\/td><td>(.*?)<\/td>/);
      const zoneMatch = desc.match(/OSS_CATCHMENT<\/td><td>(.*?)<\/td>/);

      if (idMatch) {
        const id = idMatch[1];
        if (!parks[id]) {
          parks[id] = { time: null, note: "" };
        }
        if (zoneMatch) {
          parks[id].zone = zoneMatch[1];
        }
      }
    });
  }
...
function exportPDF() {
  ...
  img.onload = function () {
    ...
    const grouped = {};
    keys.forEach(id => {
      const zone = parks[id].zone || "Unknown Zone";
      if (!grouped[zone]) grouped[zone] = [];
      grouped[zone].push({ id, ...parks[id] });
    });
    ...
    if (keys.length === 0) {
      ...
      zoneKeys.forEach(zone => {
        ...
        doc.setFontSize(12);
        y += 10;
        doc.text(`Zone ${zone}`, pageWidth / 2, y, { align: "center" });
        y += 6;

        doc.setFont("helvetica", "normal");
        doc.setFontSize(10);

        grouped[zone].forEach(park => {
          const dateFormatted = formatToAEST(park.time);
          const note = park.note || "";

          doc.text(park.id, col1, y, { align: "center" });
